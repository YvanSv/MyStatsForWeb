      ...
      <div className="max-w-[1400px] mx-auto py-12 px-6">
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <section className="flex-1">
            ...
            {viewMode === 'list' ? (
              <ListView musics={musics} sortConfig={sortConfig} onSort={handleSort} />
            ) : (
              <GridView musics={musics} sortConfig={sortConfig} onSort={handleSort} />
            )}
    ...
    <div className="space-y-4">
      ...
      {musics.map((track) => (
        <div key={track.spotify_id} className="group grid grid-cols-[2fr_120px_140px_100px_80px_60px] items-center gap-4 bg-bg2/30 backdrop-blur-sm p-4 rounded-2xl border border-white/5 hover:border-vert/30 transition-all hover:translate-x-1">
          ...
          <div className="text-center font-bold text-vert">{track.rating}</div>
        </div>
      ))}
    </div>
  );
          ...
          <div key={track.spotify_id} className="group bg-bg2/30 backdrop-blur-md rounded-3xl border border-white/5 p-4 hover:border-vert/40 transition-all hover:-translate-y-1">
            <div className="relative aspect-square mb-4 overflow-hidden rounded-2xl shadow-2xl">
              ...
              <div className="absolute top-3 right-3 bg-black/60 backdrop-blur-md px-2 py-1 rounded-lg border border-white/10">
                <span className="text-vert font-bold text-xs">{track.rating}</span>
              </div>
            </div>
