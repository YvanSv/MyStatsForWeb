...
const LIST_CELL_STYLES = {
  ...
  WRAPPER: `group flex lg:grid lg:grid-cols-[45px_2fr_120px_120px_140px_100px_60px] 
            items-center gap-3 md:gap-4 bg-bg2/30 backdrop-blur-sm p-2 rounded-2xl 
            border border-white/5 hover:border-vert/30 transition-all hover:translate-x-1`,

  // Index (#1, #2...)
  INDEX: "sm:block text-gray-500 font-mono text-sm md:text-lg",
  ...
  SUBTITLE: "text-gray-400 text-[10px] md:text-xs truncate",

  // Colonnes Desktop (Stats)
  COLUMN_DESKTOP: (isActive: boolean) => `
    hidden lg:block text-center transition-colors
    ${isActive ? 'text-medium font-bold text-vert' : 'text-sm text-gray-400'}
  `,
  ...
  MOBILE_STAT_CONTAINER: "font-hias font-bold text-sm text-vert lg:text-gray-300 lg:font-normal",
    ...
    <div className={LIST_CELL_STYLES.WRAPPER}>
      ...
      <div className="hidden lg:flex justify-center">
        <div className="flex items-center gap-2">
          <span className={sort === 'engagement' ? 'font-bold text-vert' : 'text-sm text-gray-400'}>
