...
def get_date_metadata(db: Session = Depends(get_session), current_user_id: str = ""):
    ...
    if not history_dates: return {"date_min": "1890-01-01", "date-max": datetime.now().strftime("%Y-%m-%d")}
    ...
    d_max = history_dates[1].strftime("%Y-%m-%d") if history_dates and history_dates[1] else datetime.now().strftime("%Y-%m-%d")
